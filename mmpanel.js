...
const MultiMonitorsPanel = GObject.registerClass(
    class MultiMonitorsPanel extends St.Widget {
        ...
        _updatePanel() {
            ...
            // Ensure system tray is rightmost (preserving vitalsMenu order)
        ...
        _cloneAllMainPanelIndicators() {
            ...
            // Ensure vitalsMenu is positioned before quickSettings (system controls: Network, Audio, Battery)
            let vIdx = rightIndicators.indexOf('vitalsMenu');
            let qIdx = rightIndicators.indexOf('quickSettings');
            if (vIdx >= 0 && qIdx >= 0 && vIdx > qIdx) {
                rightIndicators.splice(vIdx, 1);
                rightIndicators.splice(qIdx, 0, 'vitalsMenu');
            }
...
MultiMonitorsPanel.prototype._ensureQuickSettingsRightmost = function () {
    ...
    // Ensure vitalsMenu is placed immediately before quickSettings (Network, Audio, Battery)
    if (this.statusArea.vitalsMenu) {
        const vitalsInd = this.statusArea.vitalsMenu;
        const vitalsCont = vitalsInd.container ? vitalsInd.container : vitalsInd;
        const vitalsParent = vitalsCont.get_parent();
        if (vitalsParent) vitalsParent.remove_child(vitalsCont);
        const qsIdx = this._rightBox.get_children().indexOf(container);
        this._addToPanelBox('vitalsMenu', vitalsInd, Math.max(0, qsIdx), this._rightBox);
    }
};
